...
| `ctf_sync` | Sync challenges from the platform, create workspace directories, download files, update local state. Use `full=true` to cache descriptions/hints and auto-unlock free hints. |
...
| `ctf_queue_status` | Get the challenge priority queue, showing what to solve next, what's in progress, and what failed. Persists across agent restarts. |
...
| `ctf_save_writeup` | Save a writeup for a solved challenge, recording methodology and tools used. Generates `writeup.md` in the challenge directory. Call after submitting a flag. |
